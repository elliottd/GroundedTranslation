...
use_gpu = 1;
caffe('set_device', 1);
model_def_file = 'deploy_features-conv54.prototxt';
...
feats = zeros(512*14*14, N, 'single');
for b=1:batch_size:N
    ...
    nb = length(Is);
    feats(:, b:b+nb-1) = reshape(scores, [512*14*14,10]);
    fprintf('%d/%d = %.2f%% done in %.2fs\n', b, N, 100*(b-1)/N, tt);
end

%% write to file

save([my_path 'conv54_vgg_feats_hdf5.mat'], 'feats', '-v7.3');
