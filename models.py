...
from keras.layers.core import Dense, Activation, Dropout, Merge, TimeDistributedDense
...
class TwoLayerLSTM:
  ...
  def buildKerasModel(self):
    ...
    model.add(LSTM(self.hidden_size, self.hidden_size, return_sequences=True)) # First LSTM layer 
    ...
    model.add(LSTM(self.hidden_size, stacked_LSTM_size, return_sequences=True)) # Second LSTM layer 
    model.add(TimeDistributedDense(stacked_LSTM_size, self.vocab_size, W_regularizer=l2(self.l2reg)))
